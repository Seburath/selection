...
def parse(ticker):
    ...
    try:
        ...
        if True:
            ...
            earnings_date = f'{date.year}-{"%02d" % date.month}-{"%02d" % date.day}'
        ...
        day_range = summary_data["Day's Range"].split(" - ")
        summary_data.update({"High":day_range[1], "Low":day_range[0]})
        return summary_data
    except ValueError:
